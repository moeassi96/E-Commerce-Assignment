window.addEventListener("load", async () => {
  ...
  const productres = await fetch(
    `http://127.0.0.1:8000/api/getallproducts-cat`,
    {
      method: "GET",
    }
  );
  ...
  editBtns.forEach((button) => {
    button.addEventListener("click", async () => {
      ...
      const getinfores = await fetch(
        `http://127.0.0.1:8000/api/getProductDetails/${product_id}`,
        {
          method: "GET",
        }
      );
      ...
      document.getElementById(
        "previous-image"
      ).innerHTML = `<img style="height: inherit;" src="${productMessage.image}" alt="">`;
    ...
    if (fileInput) {
      ...
      fileReader.onload = async function (event) {
        ...
        const updateresponse = await fetch(
          `http://127.0.0.1:8000/api/updateProduct/${currentProductID}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
