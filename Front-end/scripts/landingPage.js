window.addEventListener("load", async () => {
  ...
  favoritesBtn.addEventListener("click", async () => {
    ...
    const favres = await fetch(
      `http://127.0.0.1:8000/api/getFavoritesByUserId/${user.id}`,
      {
        method: "GET",
      }
    );
    ...
    removeFavoritesButtons.forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const deleteres = await fetch(
          `http://127.0.0.1:8000/api/removeFavorite/${user.id}/${product_id}`,
          {
            method: "DELETE",
          }
        );
  ...
  function addToFavourites() {
    ...
    addtofavoritesbuttons.forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        const favres = await fetch(
          `http://127.0.0.1:8000/api/addFavorite/${user.id}/${product_id}`,
          {
            method: "GET",
          }
        );
